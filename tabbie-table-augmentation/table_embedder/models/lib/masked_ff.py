...
class MaskedFeedForward(FeedForward):
    """
    ...
    def forward(self, inputs: torch.Tensor, mask) -> torch.Tensor:
        ...
        output = inputs
        for layer, dropout in zip(self._linear_layers, self._dropout):
            ## NOTE : This is currently working for only 1 linear layer
#            output = dropout(F.softmax(F.linear(output, layer.weight[(mask!=0), :], layer.bias[mask!=0])))
            output = F.linear(output, layer.weight[(mask!=0), :], layer.bias[mask!=0])
        return output
