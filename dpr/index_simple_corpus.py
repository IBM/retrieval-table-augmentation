...
from util.corpus_reader import corpus_reader, Passage
...
def embed(doc_batch: List[Passage], ctx_encoder: DPRContextEncoder, ctx_tokenizer: DPRContextEncoderTokenizerFast) -> np.ndarray:
    documents = {"title": [doci.title for doci in doc_batch], 'text': [doci.text for doci in doc_batch]}
    """Compute the DPR embeddings of document passages"""
    input_ids = ctx_tokenizer(
        documents["title"], documents["text"], truncation=True, padding="longest",
        return_tensors="pt", max_length=opts.seq_len_c
    )["input_ids"]
    embeddings = ctx_encoder(input_ids.to(device=device), return_dict=True).pooler_output
    return embeddings.detach().cpu().to(dtype=torch.float16).numpy()


def write(cur_offset, offsets, passage_file, doc_batch: List[Passage], embeddings):
...
report = Reporting()
doc_batch = []
for pndx, passage in enumerate(corpus_reader(opts.corpus)):
    if pndx % embed_count != (embed_num-1):
        continue
    if report.is_time():
