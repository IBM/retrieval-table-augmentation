...
class ShuffledWriter:
    def __init__(self, output_dir, *, extension='.jsonl.gz', num_files=16, rand: random.Random=None):
        digits_len = math.floor(math.log10(num_files)) + 1
        format_str = f'0{digits_len}d'
        self.files = [write_open(os.path.join(output_dir, f'{i:{format_str}}{extension}')) for i in range(num_files)]
