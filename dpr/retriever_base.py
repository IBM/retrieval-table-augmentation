...
class RetrieverBase:
    ...
    def add_kd_loss(self, logits, teacher_labels, hard_loss):
        """
        compute the combined loss and kd_loss
        :param logits: torch.Tensor (batch x n_docs)
        :param teacher_labels: torch.Tensor (batch x n_docs)
        :param hard_loss: torch.Tensor scalar
        :return: combined_loss, hard_loss, kd_loss (torch.Tensor scalars)
        """
    ...
    def track_retrieval_metrics(self, positive_pids: List[List[str]], docs: List[dict], *, display_prefix=''):
    ...
    def retrieve_forward(self, queries, *, positive_pids=None, exclude_by_pid_prefix=None):
        """
        ...
        :return: doc_scores, docs and info-for-backward (when calling retrieve_backward)
        """
        doc_scores, docs, ifb = None, None, None
